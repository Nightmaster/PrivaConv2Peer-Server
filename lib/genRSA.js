...
function genPubKey(err)
{
	if (err)
		...
		cp.exec('openssl rsa -in ' + pathTo + 'id_rsa.pem -passin pass:' + pass + ' -pubout -out ' + pathTo + 'id_rsa.pub', function(err)
		{
			fs.fchmod(pathTo + 'id_rsa.pem', 600, function(err)
			{
				if (err)
					console.error(err);
				else
					fs.fchmod(pathTo + 'id_rsa.pub', 644, function(err)
					{
						if (err)
							console.error(err);
					});
