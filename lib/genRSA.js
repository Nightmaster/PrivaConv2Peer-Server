...
function genRSA(pw, length, user)
{
	...
	else if (utils.typeVerificator(pw, 'String') && utils.isInt(length))
	{
		this.user = user;
		pass = pw;
		pathTo = '/PrivaConv2Peer/' + this.user.username.toLowerCase() + '/';
...
function genPubKey(err)
{
	...
	{
		var subj = '/C=FR/ST=75/L=Paris/O=ESGI/OU=PrivaConv2Peer/CN=' + user.fullname + '/emailAddress=' + user.email;
		cp.exec('openssl req -new -x509 -keyform PEM -key ' + pathTo + 'id_rsa.pem -passin pass:' + pass + '-subj "' + subj + '" -outform DER -out  ' + pathTo + 'pub.der', function (err)
		{
...
function decryptTemp(pw, user, cb)
{
	...
	cp.exec('openssl pkcs8 -topk8 -inform PEM  -in ' + pathTo + 'id_rsa.pem -passin pass:' + pw + ' -outform DER -out ' + pathTo + 'tmp.der -nocrypt', cb);
	setTimeout(function()
	{
